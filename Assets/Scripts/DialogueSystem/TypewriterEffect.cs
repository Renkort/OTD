...
namespace Akkerman.DialogueSystem
{
    ...
    {
        [SerializeField] private bool waitOnPunctuation = false;
        ...
        private IEnumerator TypeText(string textToType, TextMeshProUGUI textLabel)
        {
            ...
            while (charIndex < textToType.Length)
            {
                ...
                for (int i = lastCharIndex; i < charIndex; i++)
                {
                    ...
                    if (waitOnPunctuation && IsPunctuation(textToType[i], out float waitTime) && !isLast && !IsPunctuation(textToType[i+1], out _))
                    {
